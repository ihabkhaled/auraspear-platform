...
import { useAiAgentEditDialog } from '@/hooks'
...
export function AiAgentEditDialog({
  ...
  const { t, register, handleSubmit, control, errors, handleFormSubmit, handleOpenChange } =
    useAiAgentEditDialog({ open, onOpenChange, onSubmit, initialValues })
      ...
      <DialogContent className="w-full max-w-[calc(100vw-2rem)] overflow-hidden sm:max-w-xl">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="flex min-w-0 flex-col gap-4">
          <div className="flex flex-col gap-2">
            <Label htmlFor="edit-agent-name">{t('fieldName')}</Label>
            <Input
              id="edit-agent-name"
              {...register('name')}
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="edit-agent-description">{t('fieldDescription')}</Label>
            <Textarea
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="edit-agent-model">{t('fieldModel')}</Label>
            <Input
              id="edit-agent-model"
              {...register('model')}
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="edit-agent-soul">{t('fieldSoulMd')}</Label>
            <Textarea
              id="edit-agent-soul"
              {...register('soulMd')}
